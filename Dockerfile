...
# Note YUM_MIRROR_BASE_URL ends up needing double escaping of backslashes as both bash and sed have `/` and `\` as special chars used for escapes.
ARG DISABLE_YUM_MIRROR=false
ARG YUM_MIRROR_BASE_URL='http://alma.mirror.ac.za'
RUN if [ "$DISABLE_YUM_MIRROR" != true ]; then exit; fi; \
  for f in /etc/yum.repos.d/*.repo; do \
    sed 's/^mirrorlist/# mirrorlist/g' -i "$f" && \
    sed "s/^# baseurl=https:\\/\\/repo.almalinux.org\\/almalinux/baseurl=${YUM_MIRROR_BASE_URL//\//\\/}/g" -i "$f"; \
  done
